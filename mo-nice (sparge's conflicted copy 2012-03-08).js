...
global.MO = {
	...
	letters : {
	    ...
	    year and month are normal i.e. 9 = September
	    letter is given as a number, 1 = A, etc.
	    */
	    2011: {
    ...
    defaultProfile: {
            ...
            blocks: {
                TAG: {name:'',title:'TAG',showInWeek:false},
                B1: 	{name:'Block 1',title:'1(A) 2(D) 3(C)',showInWeek:true},
                B2: 	{name:'Block 2',title:'1(B) 2(A) 3(D)',showInWeek:true},
                B3: 	{name:'Block 3',title:'1(C) 2(B) 3(A)',showInWeek:true},
                B4: 	{name:'Block 4',title:'1(D) 2(C) 3(B)',showInWeek:true},
                B5: 	{name:'Block 5',title:'5(A) 6(D) 7(C)',showInWeek:true},
                B6: 	{name:'Block 6',title:'5(B) 6(A) 7(D)',showInWeek:true},
                B7: 	{name:'Block 7',title:'5(C) 6(B) 7(A)',showInWeek:true},
                B8: 	{name:'Block 8',title:'5(D) 6(C) 7(B)',showInWeek:true},
                STAR:   {name:'STAR',title:'',showInWeek:true},
                Lunch:  {name:'',title:'',showInWeek:false},
            },
            matrix: {
                //letter:[block in rotator 0, 1, 2, 3, 4, 5, 6]
	...
	global.Date.prototype.getLetter = function() {
		var rtn;
		(rtn = global.MO.letters[this.getYear()+1900]) 
	...
	global.Date.prototype.getSchedule = function() {
		var rtn;
		( rtn = global.MO.schedules[this.getYear()+1900] ) 
			&& ( rtn = rtn[this.getMonth()+1] ) 
			&& ( rtn = rtn[this.getDate()] )
	...
	global.Date.prototype.isSchoolDay = function () {
		return isFinite(this.getLetter()) && (!!this.getSchedule());
	}

	global.Date.prototype.getBells = function() {
		if ( !this.isSchoolDay() ) {return undefined;}
		var letter = this.getLetter(); 
		var schedule = this.getSchedule();

		var profile = MO.defaultProfile;
		...
		for (p in schedule.periods) {
			bells[p]={};
			bells[p].period = schedule.periods[p][MO.scheduleTypes.NAME]
			bells[p].block = ( profile.matrix[letter][bells[p].period] ) || ( bells[p].period)
		}

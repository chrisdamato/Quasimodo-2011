...
window.mo = (function() {
    ...
    var dayMaker=function(date) {
            ...
            if (date==undefined) {
                if (Date.parse("now")._getMFM()>860) {
                    ...
                    date=Date.today(); //http://code.google.com/p/datejs/
                }
                var last=Date.parse('9/1/2012');
                while (!isSchoolDay(date)) {
                    date=date.next().day();
                    if (date.compareTo(last)>-1) {
                        return false;
                    }
                }
            } else {
            ...
            var user=profile.user || defaultUser();
            var matrix=profile.matrix || defaultMatrix();
            ...
            return {
                ...
                bells: function() {
                    ...
                    for (var i in schedule.items) {
                        ...
                        bell.block=user[matrix[letter][bell.name]] //rotator from user info
                            || user[bell.name] // or course from user info
    ...
    return {
        ...
        user: defaultUser(),
        defaultMatrix: defaultMatrix, //factory for customizable matrix
        defaultUser: defaultUser, //factory for customizable matrix
...
window.Day=Backbone.Model.extend({
    ...
    updateFromMO: function() {
        var oDate=(_.isUndefined(this.get('date')))? 
            undefined :
            Date.parse(this.get('date'));
        var day=mo.day(oDate,this.get("user"),this.get("matrix"));
        day.bells=day.bells()
        this.set(day);
    } 
})
...
$(document).ready(function() {
    ...
    chrisMatrix=mo.defaultMatrix()
    chrisMatrix[3].AM2=9;
    dayChris=new window.Day({user:chrisUser,matrix:chrisMatrix});
    viewChris=new DayView({model:dayChris,el:$('#chrisDay')})
    dayChris.set( { date:undefined } )
